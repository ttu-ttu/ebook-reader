...
export abstract class BaseStorageHandler {
  ...
  protected static getBookFileName(book: Omit<BooksDbBookData, 'id'> | File, existingFilename?: string) {
    ...
    if (existingFilename) {
      ...
      return `bookdata_${exporterVersion}_${currentDbVersion}_${characters ||
        BaseStorageHandler.getBookCharacters(book.characters || 0, book.sections || [])
        }_${book.lastBookModified || lastBookModified || 0}_${book.lastBookOpen || lastBookOpen || 0
        }.zip`;
    }
  ...
  protected static getProgressFileName(progress: BooksDbBookmarkData | File) {
    return progress instanceof File
      ? progress.name
      : `progress_${exporterVersion}_${currentDbVersion}_${progress.lastBookmarkModified || 0}_${progress.progress || 0
      }.json`;
  }
