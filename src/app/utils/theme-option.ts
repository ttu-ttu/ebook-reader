...
export interface ThemeOption {
  ...
  toolipTextFontColor: string;
}

function updateHintFuriganaFontColor(
  theme: Record<keyof ThemeOption, ColorObject>
) {
  return {
    ...theme,
...
function themeObjValueToStringValue<T extends string>(
  objValue: Record<T, ColorObject>
) {
  return Object.entries(objValue).reduce<Record<T, string>>(
    (acc, [key, value]) => {
...
export const availableThemes = Object.entries(availableThemesCamelCase).reduce<
  Record<string, ThemeOption>
>((acc, [key, value]) => {
  acc[camelCaseToKebabCase(key)] = themeObjValueToStringValue(value);
  return acc;
}, {});
